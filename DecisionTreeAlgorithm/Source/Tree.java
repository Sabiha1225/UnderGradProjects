...
public class Tree {
    ...
    public void createTree(List<List<Attribute>> records)
    {
        ...
        root = decisionTreeLearning(records,al.getAttributeIDList(), null);
    }
    public Node decisionTreeLearning(List<List<Attribute>> records, List<Integer> attributeIDList, Node parentNode) 
    {
        Attribute at = new Attribute();
        Node test = new Node(records, attributeIDList);
        test.setParent(parentNode);
        
        if(positiveCheck(records))
        {
            at.setLabel("1");
            test.setAtt(at);
            return test;
        }
        if(negativeCheck(records))
        {
            at.setLabel("0");
            test.setAtt(at);
            return test;
        ...
        for(int i=0; i<valueList.size(); ++i)
        {
            ...
            else
            {
                ...
                vola2 = decisionTreeLearning(subRecords, subAttIDList, vola1);
